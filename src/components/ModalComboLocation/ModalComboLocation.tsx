...
const ModalComboLocation = ({
    ...
    const [selectedLocation, setSelectedLocation]: any = useState(getDefaultLocation());
    const [selectedProvince, setSelectedProvince]: any = useState({ id: provinceId, name: provinceName });
    const [selectedDistrict, setSelectedDistrict]: any = useState({ id: districtId, name: districtName });
    const [selectedWards, setSelectedWards]: any = useState({ id: wardsId, name: wardsName });
    ...
    const scrollToItem = (location) => {
        ...
        if (refList.current && index !== -1) {
            setTimeout(() => refList.current.scrollToIndex({ index }));
        }
    }
    ...
    const getLocationInfo = async (type = selectedType, parentId = selectedParentId) => {
        const data: any = {};
    ...
    const handlePressLocation = (location) => {
        ...
        let type = selectedType, parentId = selectedParentId;
        switch (selectedType) {
            case COMBO_LOCATION_TYPE.PROVINCE:
        ...
        <ModalBox
            ...
            <FlatList
                ...
                getItemLayout={(data, index) => (
                    { length: LOCATION_HEIGHT, offset: LOCATION_HEIGHT * index, index }
                )}
