...
const PRODUCT_MESSAGE =
  'Ví Sản phẩm được rút về App vào cuối tháng.\nĐược sử dụng để mua trong cửa hàng Hương Cảng.';
...
class NdtHistory extends Component {
  ...
  _getInvestorHistory(apiType) {
    ...
    let loadingType =
      apiType === 0 ? 'loadingHistoryWithdraw' : 'loadingHistory';
      ...
      async () => {
        try {
          var response = await APIHandler[
            apiType === 0 ? 'investor_historywithdraw' : 'investor_history'
          ](data);
          let state = this.state;
  ...
  render() {
    ...
    let { loadingHistory, loadingHistoryWithdraw } = this.state;
    ...
    if (ndt_history) {
      inputHistory =
        ndt_history.data[
          type === '0' ? 'ndt_cash_input_history' : 'ndt_product_input_history'
        ];
      withdrawHistory =
        ndt_history.data[
          type === '0'
      ...
      inputHistory =
        inputHistory && inputHistory.map((d, i) => <Row data={d} key={i} />);
      withdrawHistory =
        withdrawHistory &&
        withdrawHistory.map((d, i) => <Row data={d} key={i} />);
    }
        ...
        >
          ...
          >
            <View style={{ flex: 1 }}>
              {type === '0' ? (
                <Text>
                  Thông tin lịch sử rút tiền và thu nhập ví Tiền mặt trên Web
                  Nhà đầu tư
                </Text>
              ) : (
                <Text>
                  Thông tin lịch sử rút tiền và thu nhập ví Sản phẩm trên Web
                  Nhà đầu tư
                </Text>
              )}
            </View>
            {type === '0' && (
              <View style={{ flex: 1 }}>
                ...
                >
                  <View
                    style={[styles.empty_box_btn, isIOS && { height: '100%' }]}
                  >
      ...
      >
        <View style={[styles.row]}>
          ...
          <View style={[styles.contents, styles.middle]}>
            ...
            <Text style={[styles.des]}>{formatDate(DateGet)}</Text>
          </View>
        </View>
        <View style={[styles.contents, styles.end]}>
          <Text style={[styles.money]}>{`+${Money.formatMoney()}đ`}</Text>
        </View>
      </Animated.View>
    );
  }
